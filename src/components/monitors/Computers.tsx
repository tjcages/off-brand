...
function Screen({
  ...
  return (
    <group position={position} rotation={rotation} scale={scale}>
      <mesh castShadow receiveShadow geometry={frameGeometry} material={materials.Texture} />
...
function ScreenText({
  ...
  y?: number;
  video?: string;
  setZoom?: (arg0: boolean) => void;
}) {
  const textRef = useRef() as any;
  const rand = Math.random() * 10000;
  const [videoVisible, set] = useState(false);
  useFrame(
    state =>
      !videoVisible &&
  ...
  useEffect(() => {
    if (!video) return;
    const timeout = setTimeout(() => set(true), Math.random() * 10000);
    return () => clearTimeout(timeout);
  }, [video]);

  return (
    <Screen frame={frame} panel={panel} position={position} rotation={rotation} scale={scale}>
      <PerspectiveCamera makeDefault manual aspect={1 / 1} position={[0, 0, 15]} />
      <color attach="background" args={[invert ? "black" : "#0000ff"]} />
      <ambientLight intensity={0.5} />
      <directionalLight position={[10, 10, 5]} />
      <SpinningBox position={[-3.15, 0.75, 0]} scale={0.5} setZoom={setZoom} />
      {video !== undefined && videoVisible ? (
        <mesh position={[0, 1, 0]}>
        ...
        <>
          <Image
            url="https://dwvo2npct47gg.cloudfront.net/images/2020/flacko-2.jpg"
            // @ts-expect-error no alt prop needed
            alt="rocky"
            scale={[5, 5]}
          />
          <Text
...
function ScreenInteractive(props: any) {
  return (
    <Screen {...props}>
      ...
      <SpinningBox position={[-3.15, 0.75, 0]} scale={0.5} setZoom={props.setZoom} />
    </Screen>
  );
}
