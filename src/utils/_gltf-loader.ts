...
/* load & process */
export function loadModelDeep(url: DataType) {
  return new Promise((resolve, _) => {
    load(url, GLTFLoader).then(data => {
      const arr: {
        name: string;
        data: { position: DataType; normal: DataType; texcoord: DataType; indices: DataType };
        arr: {
          position: { numComponents: number; data: number[] };
          normal: { numComponents: number; data: number[] };
          texcoord: { numComponents: number; data: number[] };
          indices: { numComponents: number; data: number[] };
        };
      }[] = [];

      // @ts-expect-error - meshes doesn't exist on type 'DataType'
